...
from futbot.branding import CONSOLE_NAME, FOOTER
...
def player_embed(quote: PlayerQuote, title: str | None = None) -> discord.Embed:
    ...
    embed.add_field(name=CONSOLE_NAME, value=format_coins(quote.ps5.price), inline=True)
    if player.image_url:
...
def compare_embed(left: PlayerQuote, right: PlayerQuote) -> discord.Embed:
    ...
    embed.add_field(
        name=f"Differenz {CONSOLE_NAME}",
        value=_diff(left.ps5.price, right.ps5.price),
        inline=False,
    )
...
def _player_description(player: PlayerCard) -> str:
    bits = [part for part in (player.club, player.nation, player.league, CONSOLE_NAME) if part]
    return " · ".join(bits) if bits else f"EA FC 27 · {CONSOLE_NAME}"


def _quote_block(quote: PlayerQuote) -> str:
    return (
        f"{quote.player.club or '—'}\n"
        f"{CONSOLE_NAME}: {format_coins(quote.ps5.price)}"
    )
